...
export default class UrlPage extends Component {
                ...
                if(result.success) {
                    this.setState({
                        ...
                        url: {
                            ...
                            customShortUrl: this.state.url.customShortUrl,
                            shortUrl: result.url.shortUrl,
                        }
                    });
                }
                else {
                    this.setState({ modalDanger: true, error: result.error.message });
                }
            })
            .catch(e => {
                console.log(e);
    ...
    async getShortUrl() {
        try {
            return await fetch('http://94.142.141.165/custom/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
                body: "originalUrl=" + this.state.url.originalUrl + '&customShortUrl=' + this.state.url.customShortUrl,
                mode: 'cors'
            });
        } catch(error) {
            console.error(error);
